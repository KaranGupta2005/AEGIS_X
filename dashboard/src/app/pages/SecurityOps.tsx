...
const STATE_COLORS: Record<string, string> = {
  ...
  CONTAINMENT: '#EF4444', VERIFICATION: '#8B5CF6', RECOVERY: '#10B981', TERMINATED: 'var(--text-muted)',
}
...
const SecurityOps: React.FC = () => {
  ...
  const stateColor = STATE_COLORS[secState] || 'var(--text-muted)'
  ...
  return (
    <div style={{ padding: '20px 24px', height: 'calc(100vh - 80px)', overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-main)', fontFamily: 'Space Grotesk' }}>Security Operations</div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>Deception & Secure Transaction Sandbox</div>
        </div>
        {/* Live state badge */}
      ...
      {/* Security State Flow */}
      <div style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
        <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 10, textTransform: 'uppercase' }}>State Machine</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        <div style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 8, textTransform: 'uppercase' }}>Threat Reasons</div>
          {(secStatus?.reasons || ['No threats detected']).map((r: string, i: number) => (
            <div key={i} style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'Space Grotesk', padding: '3px 0', borderBottom: '1px solid var(--border-light)' }}>• {r}</div>
          ))}
        </div>
        <div style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 12, padding: 14 }}>
          <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 8, textTransform: 'uppercase' }}>Bot Indicators</div>
      ...
      {/* Policy Info */}
      <div style={{ marginTop: 16, background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 12, padding: 14 }}>
        <div style={{ fontSize: 9, fontWeight: 700, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 8, textTransform: 'uppercase' }}>Trust Policy</div>
        <div style={{ fontSize: 10, fontFamily: 'JetBrains Mono', color: 'var(--text-muted)', lineHeight: 2 }}>
          ...
          <div>▸ Critical → <span style={{ color: 'var(--text-muted)' }}>TERMINATED</span> (all operations frozen)</div>
        </div>
      </div>
    </div>
  )
