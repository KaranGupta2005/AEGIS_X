...
function Section({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: 20 }}>
      <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: '0.12em', color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', textTransform: 'uppercase', marginBottom: 10 }}>{title}</div>
      {children}
    </div>
  )
}
...
function MetricCard({ label, value, color, icon: Icon }: { label: string; value: string; color: string; icon?: any }) {
  return (
    <div style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 12, padding: '14px 16px' }}>
  ...
  return (
    <div style={{ padding: '20px 24px', height: 'calc(100vh - 80px)', overflowY: 'auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontSize: 18, fontWeight: 800, color: 'var(--text-main)', fontFamily: 'Space Grotesk' }}>Verification Center</div>
          <div style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>Adaptive Identity Verification Framework</div>
        </div>
      </div>

      {/* Tabs */}
      <div style={{ display: 'flex', gap: 4, marginBottom: 20, borderBottom: '1px solid var(--border-light)', paddingBottom: 1 }}>
        {tabs.map(tab => {
          const Icon = tab.icon
        ...
        {activeTab === 'overview' && (
          <motion.div key="overview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            ...
            <Section title="Registered Providers">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                {providers && Object.entries(providers).map(([key, value]) => (
                  <div key={key} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 12px', background: 'var(--accent-dim)', borderRadius: 8, border: '1px solid var(--border-light)' }}>
                    <span style={{ fontSize: 10, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>{key.replace(/_/g, ' ')}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, color: value ? '#10B981' : 'var(--text-muted)', fontFamily: 'Space Grotesk' }}>{value || 'Not registered'}</span>
                  </div>
                ))}
              </div>
            </Section>
            ...
            <Section title="Verification Policy">
              <div style={{ fontSize: 10, fontFamily: 'JetBrains Mono', color: 'var(--text-muted)', lineHeight: 2 }}>
...
function EnrollmentWizard({ userId }: { userId: string }) {
  ...
  return (
    <div>
      ...
      {/* Step content */}
      <motion.div key={step} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} style={{ background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 14, padding: 24, minHeight: 200 }}>
        <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text-main)', fontFamily: 'Space Grotesk', marginBottom: 6 }}>{ENROLLMENT_STEPS[currentIdx].label}</div>
        <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'Space Grotesk', marginBottom: 20 }}>{ENROLLMENT_STEPS[currentIdx].description}</div>
        ...
        {step === 'complete' && <div style={{ textAlign: 'center', padding: 20 }}><CheckCircle size={48} color="#10B981" /><div style={{ fontSize: 16, fontWeight: 700, color: '#10B981', fontFamily: 'Space Grotesk', marginTop: 12 }}>Enrollment Complete</div><div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 6 }}>All biometric profiles are ready for adaptive verification.</div></div>}
      </motion.div>

      {/* Navigation */}
      <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 16 }}>
        <button onClick={prev} disabled={currentIdx === 0} style={{ padding: '8px 20px', borderRadius: 8, border: '1px solid var(--border-medium)', background: 'transparent', color: currentIdx === 0 ? 'rgba(255,255,255,0.2)' : 'rgba(255,255,255,0.6)', fontSize: 11, cursor: currentIdx === 0 ? 'default' : 'pointer', fontFamily: 'Space Grotesk' }}>Previous</button>
        {step !== 'complete' && <button onClick={next} style={{ padding: '8px 20px', borderRadius: 8, border: 'none', background: '#8B5CF6', color: 'var(--text-main)', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'Space Grotesk' }}>Next Step</button>}
      </div>
    </div>
  )
}
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
      {['Full Name', 'Email', 'Phone', 'Account ID'].map(field => (
        <div key={field}>
          <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 4 }}>{field}</div>
          <div style={{ padding: '8px 12px', borderRadius: 8, border: '1px solid var(--border-light)', background: 'var(--accent-dim)', fontSize: 11, color: 'var(--text-sub)', fontFamily: 'Space Grotesk' }}>
...
function PlaceholderStep({ icon: Icon, text }: { icon: any; text: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', padding: 16 }}>
      ...
      <div style={{ fontSize: 11, color: 'var(--text-muted)', fontFamily: 'Space Grotesk', lineHeight: 1.7, maxWidth: 400 }}>{text}</div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div style={{ fontSize: 12, color: 'var(--text-sub)', fontFamily: 'Space Grotesk' }}>
          {delegates.length} / 3 delegates registered
        </div>
        {delegates.length < 3 && (
          <button onClick={() => setAdding(true)} style={{ padding: '6px 14px', borderRadius: 8, border: 'none', background: '#8B5CF6', color: 'var(--text-main)', fontSize: 10, fontWeight: 700, cursor: 'pointer', fontFamily: 'Space Grotesk' }}>+ Add Delegate</button>
        )}
      </div>
      ...
      {adding && (
        <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} style={{ background: 'rgba(139,92,246,0.04)', border: '1px solid rgba(139,92,246,0.2)', borderRadius: 12, padding: 16, marginBottom: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 12 }}>
            <div>
              <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 4 }}>Full Name</div>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Enter name" style={{ width: '100%', padding: '7px 10px', borderRadius: 6, border: '1px solid var(--border-medium)', background: 'var(--accent-dim)', color: 'var(--text-main)', fontSize: 11, fontFamily: 'Space Grotesk', outline: 'none' }} />
            </div>
            <div>
              <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginBottom: 4 }}>Relationship</div>
              <input value={relationship} onChange={e => setRelationship(e.target.value)} placeholder="e.g. spouse, parent" style={{ width: '100%', padding: '7px 10px', borderRadius: 6, border: '1px solid var(--border-medium)', background: 'var(--accent-dim)', color: 'var(--text-main)', fontSize: 11, fontFamily: 'Space Grotesk', outline: 'none' }} />
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={handleAdd} style={{ padding: '6px 14px', borderRadius: 6, border: 'none', background: '#10B981', color: 'var(--text-main)', fontSize: 10, fontWeight: 700, cursor: 'pointer' }}>Confirm</button>
            <button onClick={() => setAdding(false)} style={{ padding: '6px 14px', borderRadius: 6, border: '1px solid var(--border-medium)', background: 'transparent', color: 'var(--text-muted)', fontSize: 10, cursor: 'pointer' }}>Cancel</button>
          </div>
        </motion.div>
      )}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {delegates.map(d => (
          <div key={d.delegate_id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 14px', background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 10 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-main)', fontFamily: 'Space Grotesk' }}>{d.name}</div>
              <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>{d.relationship} · {d.is_active ? 'Active' : 'Disabled'}</div>
            </div>
        ...
        {delegates.length === 0 && (
          <div style={{ textAlign: 'center', padding: 24, color: 'var(--text-muted)', fontSize: 11, fontFamily: 'Space Grotesk' }}>No trusted delegates registered. Add up to 3 delegates.</div>
        )}
      </div>
    </div>
  )
...
function VerificationHistoryView({ history }: { history: VerificationChallenge[] }) {
  if (history.length === 0) {
    return <div style={{ textAlign: 'center', padding: 40, color: 'var(--text-muted)', fontSize: 11 }}>No verification history yet.</div>
  }
      ...
      {history.slice().reverse().map((v, i) => {
        ...
        return (
          <motion.div key={i} initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
            style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', background: 'var(--accent-dim)', border: '1px solid var(--border-light)', borderRadius: 10 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-main)', fontFamily: 'Space Grotesk' }}>{v.verification_type.replace(/_/g, ' ')}</span>
                <span style={{ fontSize: 9, fontWeight: 700, color: statusColor, fontFamily: 'JetBrains Mono' }}>{v.status}</span>
              </div>
              <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginTop: 3 }}>
                Risk: {v.risk_source} · Confidence: {Math.round(v.confidence * 100)}% · Latency: {v.latency_ms.toFixed(0)}ms
              </div>
              <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'Space Grotesk', marginTop: 3 }}>
                Trust: {(v.trust_before * 100).toFixed(0)}% → {(v.trust_after * 100).toFixed(0)}%
              </div>
              {v.reason && <div style={{ fontSize: 9, color: 'var(--text-muted)', fontFamily: 'JetBrains Mono', marginTop: 3, fontStyle: 'italic' }}>{v.reason}</div>}
            </div>
          </motion.div>
        )
      })}
