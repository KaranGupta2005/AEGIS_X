  ...
  return (
    <div style={{ marginBottom: 6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
        <span style={{ fontSize: 8, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>{label}</span>
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{
        ...
        color: '#5b8cc7', fontFamily: 'JetBrains Mono',
...
function MetricRow({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '3px 0', borderBottom: '1px solid var(--accent-dim)' }}>
      <span style={{ fontSize: 9, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>{label}</span>
...
export const AegisConsole: React.FC<AegisConsoleProps> = ({ state, currentPage }) => {
  ...
  const trustColor = trustScore > 85 ? '#3B82F6' : trustScore > 60 ? '#F59E0B' : '#EF4444'
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div>
          <div style={{ fontSize: 11, fontWeight: 800, color: '#e8f0fe', fontFamily: 'Space Grotesk', lineHeight: 1.2 }}>AEGIS-X</div>
          <div style={{ fontSize: 7, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>Behavioral Trust Engine</div>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
          ...
          <span style={{ fontSize: 7, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 7, color: '#5b8cc7', fontFamily: 'JetBrains Mono', marginTop: 2 }}>T R U S T · T ( t )</div>
        </div>
        ...
        >
          <div style={{ fontSize: 11, fontWeight: 900, color: decision === 'ALLOW' ? '#3B82F6' : decision === 'STEP_UP' ? '#F59E0B' : '#EF4444', fontFamily: 'Space Grotesk' }}>
            {decision}
          </div>
          <div style={{ fontSize: 7, color: '#5b8cc7', fontFamily: 'JetBrains Mono', marginTop: 1 }}>
      ...
      <div style={{
        ...
        {TABS.map(tab => {
          ...
          return (
            ...
            >
              <Icon size={12} color={active ? '#3B82F6' : 'rgba(255,255,255,0.25)'} />
              <span style={{ fontSize: 8, color: active ? '#3B82F6' : 'rgba(255,255,255,0.25)', fontFamily: 'JetBrains Mono', fontWeight: active ? 700 : 400 }}>
                {tab.label}
              </span>
            </button>
          )
          ...
          {activeTab === 'trust' && (
            <motion.div key="trust" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Section title="Temporal Dynamics">
                <MetricRow label="Velocity dT/dt" value={velocity.toFixed(4)} color={velocity < -0.01 ? '#EF4444' : '#3B82F6'} />
                ...
                <MetricRow label="Drift Severity" value={driftSeverity.toUpperCase()} color={driftDetected ? '#F59E0B' : '#3B82F6'} />
                <MetricRow label="Pipeline Latency" value={`${latencyMs.toFixed(0)}ms`} color="#8B5CF6" />
                <MetricRow label="Behavioral Similarity" value={`${(similarity * 100).toFixed(1)}%`} color={similarity > 0.85 ? '#3B82F6' : similarity > 0.65 ? '#F59E0B' : '#EF4444'} />
              </Section>
          ...
          {activeTab === 'cognitive' && (
            <motion.div key="cognitive" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Section title="Behavioral Biometrics">
                <MetricRow label="Typing Speed" value={`${(state.timeline.length > 0 ? (state.timeline[state.timeline.length - 1] as any)?.typing_speed ?? '—' : '—')} cps`} color="rgba(255,255,255,0.7)" />
                <MetricRow label="Hesitation Ratio" value={cognitiveState === 'calm' ? 'LOW' : cognitiveState === 'focused' ? 'MINIMAL' : cognitiveState === 'distressed' ? 'ELEVATED' : cognitiveState === 'panicked' ? 'HIGH' : cognitiveState === 'coerced' ? 'EXTREME' : 'ZERO'} color={cognitiveState === 'calm' || cognitiveState === 'focused' ? '#3B82F6' : cognitiveState === 'distressed' ? '#F59E0B' : '#EF4444'} />
                <MetricRow label="Correction Rate" value={cognitiveState === 'coerced' || cognitiveState === 'panicked' ? 'HIGH' : cognitiveState === 'distressed' ? 'MODERATE' : 'NORMAL'} color={cognitiveState === 'coerced' || cognitiveState === 'panicked' ? '#EF4444' : cognitiveState === 'distressed' ? '#F59E0B' : '#3B82F6'} />
                <MetricRow label="Gyroscope Variance" value={cognitiveState === 'robotic' ? 'FLAT (0.001)' : cognitiveState === 'coerced' || cognitiveState === 'panicked' ? 'ELEVATED' : 'NATURAL'} color={cognitiveState === 'robotic' ? '#8B5CF6' : cognitiveState === 'coerced' || cognitiveState === 'panicked' ? '#F59E0B' : '#3B82F6'} />
                <MetricRow label="Touch Duration" value={cognitiveState === 'calm' ? '~120ms' : cognitiveState === 'focused' ? '~90ms' : cognitiveState === 'panicked' ? '~320ms' : cognitiveState === 'robotic' ? '~48ms' : '~180ms'} color="rgba(255,255,255,0.6)" />
                <MetricRow label="Swipe Straightness" value={cognitiveState === 'robotic' ? '0.99 (PERFECT)' : cognitiveState === 'coerced' ? '0.42 (ERRATIC)' : cognitiveState === 'panicked' ? '0.55' : '0.82 (NATURAL)'} color={cognitiveState === 'robotic' ? '#EF4444' : cognitiveState === 'coerced' ? '#F59E0B' : '#3B82F6'} />
              </Section>

              <Section title="SDK Monitoring">
                <div style={{ fontSize: 9, color: '#5b8cc7', fontFamily: 'JetBrains Mono', lineHeight: 1.7 }}>
                  <div>▸ SDK State: <span style={{ color: SDK_STATE_COLORS[sdkState] }}>{sdkState}</span></div>
                  <div>▸ Current Screen: <span style={{ color: '#3B82F6' }}>{(currentPage ?? 'home').toUpperCase()}</span></div>
                  <div>▸ Activity: <span style={{ color: '#5b8cc7' }}>{liveActivity?.currentActivity ?? '—'}</span></div>
                  ...
                  <div>▸ Adaptive Verification: <span style={{ color: decision === 'STEP_UP' ? '#F59E0B' : '#3B82F6' }}>{decision === 'STEP_UP' ? 'ACTIVE' : 'STANDBY'}</span></div>
                </div>
              </Section>
              ...
              <Section title="Adaptive Learning">
                ...
                <MetricRow
                  ...
                  color={(liveActivity?.behaviorConfidence ?? 0) > 0.7 ? '#3B82F6' : '#F59E0B'}
                />
                <MetricRow
                  ...
                  color={
                    (liveActivity?.behaviorConfidence ?? 0) > 0.8 ? '#3B82F6'
                ...
                <MetricRow
                  ...
                  color={trustScore > 90 ? '#3B82F6' : trustScore > 70 ? '#F59E0B' : '#EF4444'}
                />
              </Section>
            </motion.div>
          )}
            ...
            <motion.div key="session" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              ...
              <Section title="Live Session Status">
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginBottom: 8 }}>
                  {[
                    ...
                    { label: 'Current Screen', value: (currentPage ?? 'home').toUpperCase(), color: '#3B82F6' },
                    ...
                    <div key={item.label} style={{
                      ...
                      <div style={{ fontSize: 7, color: '#5b8cc7', fontFamily: 'JetBrains Mono', marginBottom: 3 }}>{item.label}</div>
              ...
              <Section title="Current Activity">
                <div style={{
                  ...
                  <div style={{ fontSize: 9, color: '#93b4e4', fontFamily: 'Space Grotesk', lineHeight: 1.5 }}>
              ...
              <Section title="Behavior Confidence">
                <ConfidenceBar
                  ...
                  color={
                    (liveActivity?.behaviorConfidence ?? 0) > 0.7 ? '#3B82F6'
                ...
                <MetricRow
                  ...
                  color={trustScore > 80 ? '#3B82F6' : trustScore > 60 ? '#F59E0B' : '#EF4444'}
                />
                <MetricRow
                  label="Risk Level"
                  value={state.trustLevel?.toUpperCase() ?? 'HIGH'}
                  color={state.trustLevel === 'high' ? '#3B82F6' : state.trustLevel === 'elevated' ? '#F59E0B' : '#EF4444'}
                />
              </Section>
              ...
              <Section title="Session Timeline">
                <div style={{ position: 'relative', paddingLeft: 12 }}>
                  ...
                  {state.timeline.slice(-8).reverse().map((entry, i) => (
                    <div key={i} style={{ display: 'flex', gap: 8, marginBottom: 6, alignItems: 'flex-start' }}>
                      <div style={{
                        ...
                        background: entry.decision === 'ALLOW' ? '#3B82F6' : entry.decision === 'STEP_UP' ? '#F59E0B' : '#EF4444',
                      }} />
                      <div style={{ flex: 1 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                          <span style={{ fontSize: 8, fontWeight: 700, color: '#93b4e4', fontFamily: 'Space Grotesk' }}>
                        ...
                        <div style={{ display: 'flex', gap: 4, marginTop: 2 }}>
                          ...
                          <span style={{ fontSize: 7, color: entry.sdk_state ? SDK_STATE_COLORS[entry.sdk_state] : 'var(--text-muted)', fontFamily: 'JetBrains Mono' }}>
                            {entry.sdk_state ?? '—'}
                          </span>
                        </div>
                      </div>
                  ...
                  {state.timeline.length === 0 && (
                    <div style={{ fontSize: 8, color: '#5b8cc7', fontFamily: 'JetBrains Mono', paddingLeft: 12 }}>
              ...
              <Section title="Threat Probabilities">
                {[
                  ...
                  <div key={item.label} style={{ marginBottom: 7 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 2 }}>
                      <span style={{ fontSize: 8, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>{item.label}</span>
            ...
            <motion.div key="events" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
              <Section title="Live Behavior Stream">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                  <span style={{ fontSize: 8, color: '#5b8cc7', fontFamily: 'JetBrains Mono' }}>
                    {eventCount} windows · 2s cadence
                  </span>
                  <span style={{ fontSize: 7, color: '#3B82F6', fontFamily: 'JetBrains Mono' }}>CONTINUOUS</span>
                </div>
              ...
              <Section title="Pipeline Health">
                ...
                <MetricRow label="Cognitive Model" value="HGB V2 · 80.8%" color="#3B82F6" />
                <MetricRow label="Anomaly Detector" value="IsolationForest V2" color="rgba(255,255,255,0.4)" />
                <MetricRow label="Drift Detector" value="CUSUM (k=0.02, h=0.08)" color="rgba(255,255,255,0.4)" />
                <MetricRow label="Training Data" value="50K samples" color="rgba(255,255,255,0.4)" />
              </Section>
            </motion.div>
          )}
        </AnimatePresence>
