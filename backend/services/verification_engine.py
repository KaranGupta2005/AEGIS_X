...
class AdaptiveVerificationEngine:
    ...
    # Fix #6: Encrypt biometric profiles at rest using Fernet (AES-128-CBC)
    # Key derived from AEGISX_SESSION_SECRET via SHA-256 → base64
    # ═══════════════════════════════════════════════════════════════════════

    @staticmethod
    def _get_encryption_key() -> bytes:
        """Derive a Fernet-compatible key from the session secret."""
        import os
        import hashlib
        import base64
        secret = os.getenv("AEGISX_SESSION_SECRET", "aegisx_hackathon_2026_secret")
        # Fernet requires 32 url-safe base64 bytes → derive from SHA-256
    ...
    def _encrypt_data(self, plaintext: str) -> str:
        ...
        try:
            ...
            # Fallback: simple XOR with secret if cryptography not installed
    ...
    def _xor_encrypt(plaintext: str) -> str:
        """Fallback XOR encryption if cryptography library not available."""
        import os
        import base64
        secret = os.getenv("AEGISX_SESSION_SECRET", "aegisx_hackathon_2026_secret").encode()
    ...
    def _xor_decrypt(ciphertext: str) -> str:
        ...
        if not ciphertext.startswith("XOR:"):
            return ciphertext  # Not encrypted, return as-is (legacy data)
        secret = os.getenv("AEGISX_SESSION_SECRET", "aegisx_hackathon_2026_secret").encode()
    ...
    def _load_profiles(self):
        """Load enrolled profiles from disk. Decrypts at rest (Fix #6)."""
        self.PROFILES_DIR.mkdir(parents=True, exist_ok=True)
        
        # Load voice profiles
        voice_file = self.PROFILES_DIR / "voice_profiles.json"
        if voice_file.exists():
            try:
                raw = voice_file.read_text()
                # Try decrypting (encrypted data)
                decrypted = self._decrypt_data(raw)
                if decrypted:
                    data = json_lib.loads(decrypted)
                else:
                    # Fallback: try reading as plain JSON (legacy unencrypted)
                    data = json_lib.loads(raw)
                for uid, profile_data in data.items():
                    embedding = np.array(profile_data["embedding"], dtype=np.float32) if profile_data.get("embedding") else None
                    self._voice_profiles[uid] = VoiceProfile(
                        user_id=uid,
                        embedding=embedding,
                ...
                print(f"[AEGIS-X] Loaded {len(self._voice_profiles)} voice profiles from disk.")
            except Exception as e:
                print(f"[AEGIS-X] Warning: Could not load voice profiles: {e}")

        # Load face profiles
        face_file = self.PROFILES_DIR / "face_profiles.json"
        if face_file.exists():
            try:
                raw = face_file.read_text()
                decrypted = self._decrypt_data(raw)
                if decrypted:
                    data = json_lib.loads(decrypted)
                else:
                ...
                for uid, profile_data in data.items():
                    embedding = np.array(profile_data["embedding"], dtype=np.float32) if profile_data.get("embedding") else None
                    self._face_profiles[uid] = FaceProfile(
                        user_id=uid,
                        embedding=embedding,
                        sample_count=profile_data.get("sample_count", 1),
                        created_at=profile_data.get("created_at", ""),
                ...
                print(f"[AEGIS-X] Loaded {len(self._face_profiles)} face profiles from disk.")
            except Exception as e:
                print(f"[AEGIS-X] Warning: Could not load face profiles: {e}")

    def _persist_profiles(self):
        """Save enrolled profiles to disk. Encrypts at rest (Fix #6)."""
        self.PROFILES_DIR.mkdir(parents=True, exist_ok=True)

        # Save voice profiles (encrypted)
        voice_data = {}
        for uid, profile in self._voice_profiles.items():
            voice_data[uid] = {
        ...
        encrypted_voice = self._encrypt_data(json_lib.dumps(voice_data))
        (self.PROFILES_DIR / "voice_profiles.json").write_text(encrypted_voice)

        # Save face profiles (encrypted)
        face_data = {}
        for uid, profile in self._face_profiles.items():
            face_data[uid] = {
                "embedding": profile.embedding.tolist() if profile.embedding is not None else None,
                "sample_count": profile.sample_count,
                "created_at": profile.created_at,
        ...
        encrypted_face = self._encrypt_data(json_lib.dumps(face_data))
        (self.PROFILES_DIR / "face_profiles.json").write_text(encrypted_face)
    ...
    def enroll_voice(self, user_id: str, embedding: np.ndarray) -> Dict:
        """Enroll or update a user's voice profile. Persists to disk."""
        profile = self._voice_profiles.get(user_id)
        if profile and profile.embedding is not None:
            # Check dimension match — if different, REPLACE (don't EMA blend different sizes)
            if profile.embedding.shape != embedding.shape:
                # Dimension changed (e.g., hash fallback → real SpeechBrain), replace entirely
                norm = np.linalg.norm(embedding)
                profile.embedding = (embedding / norm if norm > 0 else embedding).astype(np.float32)
            else:
                # EMA update existing profile (same dimension)
                decay = 0.90
    ...
    def enroll_face(self, user_id: str, embedding: np.ndarray) -> Dict:
        """Enroll or update a user's face profile. Persists to disk."""
