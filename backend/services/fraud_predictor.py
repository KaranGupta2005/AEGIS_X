...
class FraudPredictor:
    ...
    def _estimate_takeover(self, similarity: float, drift: bool, severity: str, velocity: float) -> float:
        base = max(0.0, 1.0 - similarity)

        if drift and severity in ("high", "critical"):
            base = min(1.0, base + 0.3)
        elif drift and severity == "medium":
            base = min(1.0, base + 0.15)
    ...
    def _estimate_anomaly_severity(self, anomaly_score: float, drift_severity: str) -> float:
        severity_map = {"none": 0.0, "low": 0.2, "medium": 0.5, "high": 0.75, "critical": 0.95}
    ...
    def _classify_trajectory(self, trust: float, velocity: float) -> str:
        if abs(velocity) < 0.005 and trust > 0.80:
            return "stable"
        elif velocity < -0.03:
            return "collapsing"
        elif velocity < -0.01:
