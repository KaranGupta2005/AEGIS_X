...
class SecurityPolicy:
    ...
    trust_normal: float = 0.78
    trust_monitoring: float = 0.65
    trust_suspicious: float = 0.50
    trust_containment: float = 0.45
    trust_critical: float = 0.25
    bot_confidence_threshold: float = 0.60
    max_requests_per_second: float = 10.0
    max_failed_verifications: int = 3
    sandbox_duration_seconds: float = 300.0
    containment_cooldown_seconds: float = 45.0
...
class SessionContainmentService:
    ...
    def evaluate_and_update(
        ...
        # Auto-transition based on threat — progressive escalation
        # If recommended state isn't directly reachable, step through intermediates
        if recommended != ctx.state and ctx.state != SecurityState.TERMINATED:
            ...
            if recommended in allowed:
                ...
                # Progressive escalation: step through intermediate states
                escalation_path = [
                ...
                current_idx = escalation_path.index(ctx.state) if ctx.state in escalation_path else 0
                target_idx = escalation_path.index(recommended) if recommended in escalation_path else current_idx

                # Step up through each intermediate state
                if target_idx > current_idx:
                    for step in range(current_idx + 1, target_idx + 1):
                        next_state = escalation_path[step]
                        step_allowed = VALID_TRANSITIONS.get(ctx.state, [])
                        if next_state in step_allowed:
                            self.transition(ctx, next_state, f"Auto-escalate: {'; '.join(threat['reasons'])}")
                        else:
                            break  # Can't continue escalation
    ...
    def is_user_sandboxed(self, user_id: str) -> bool:
        """
        Fix #10: Check if ANY session for this user is sandboxed.
        Iterates all tracked sessions and returns True if at least one is active sandbox.
        Used by the SandboxEnforcementMiddleware.
        """
        for key, ctx in self._sessions.items():
            if ctx.user_id == user_id and ctx.sandbox_active:
                return True
        return False
